...
const StoreItem = ({ item, handleDeleteItem }) => {
    ...
    return (
        <TableRow>
            ...
            <TableData>{item.shortName}</TableData>
            <TableData center>{item.openTime}</TableData>
            <TableData center>{item.closeTime}</TableData>
            ...
            <TableData center>
                ...
                <Modal isOpen={DeleteModal} onRequestClose={toggleModal} style={customStyles}>
                    ...
                    <Row><Text>Bạn có chắc chắn muốn xóa cửa hàng【<Name>{item.name}</Name>】?</Text></Row>
